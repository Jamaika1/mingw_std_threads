...
struct FutureStatic
{
  ...
  static std::vector<std::pair<std::mutex, condition_variable> > sync_pool;

  static std::mutex & get_mutex (void const * ptr)
  {
...
template<bool b>
std::vector<std::pair<std::mutex, condition_variable> > FutureStatic<b>::sync_pool (std::thread::hardware_concurrency() * 2 + 1);

struct FutureStateBase
{
  inline std::mutex & get_mutex (void) const
  {
...
struct FutureBase : public FutureStatic<true>
{
  ...
  std::mutex & get_mutex (void) const
  {
...
//  Need to turn the pseudohandle from GetCurrentThread() into a true handle...
    ...
    try {
      ...
      std::thread watcher_thread ([ptr, thread_handle, &handle_handled](void)
        {
          {
            std::lock_guard<std::mutex> guard (ptr->get_mutex());
            handle_handled = true;
          }
          ptr->get_condition_variable().notify_all();
//  Wait for the original thread to die.
      ...
      {
        std::unique_lock<std::mutex> guard (ptr->get_mutex());
...
template<>
class shared_future<void> : private shared_future<mingw_stdthread::detail::Empty>
{
  typedef mingw_stdthread::detail::Empty Empty;
 public:
...
struct StorageHelper
{
  ...
  static void store (FutureState<Ret> * state_ptr, Func && func, Args&&... args)
  {
    try {
      ...
      std::lock_guard<std::mutex> lock { state_ptr->get_mutex() };  //  Reveal the results of the above action to
...
struct StorageHelper<Ref&>
{
  ...
  static void store (FutureState<void*> * state_ptr, Func && func, Args&&... args)
  {
    try {
      typedef typename std::remove_cv<Ref>::type Ref_non_cv;
      Ref & rf = invoke(std::forward<Func>(func), std::forward<Args>(args)...);
      std::lock_guard<std::mutex> lock { state_ptr->get_mutex() };
      state_ptr->set_value(const_cast<Ref_non_cv *>(std::addressof(rf)));
    } catch (...) {
      std::lock_guard<std::mutex> lock { state_ptr->get_mutex() };
      state_ptr->set_exception(std::current_exception());
